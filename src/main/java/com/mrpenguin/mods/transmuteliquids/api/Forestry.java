...
public class Forestry {
   ...
   public static CrucibleRecipe refractorySeedOilRecipe;
	...
	public static void thaumonomiconResearch() {
		...
		text = "1";
		research = new TLResearchItem("Transmute Juice", "Transmute Liquids", new AspectList(), -3, 4, 1, ForestryItem.canJuice.getItemStack(1)).setPages(new ResearchPage(canJuiceText), new ResearchPage(waxCapsuleJuiceRecipe),
				new ResearchPage(refractoryJuiceRecipe), new ResearchPage(canJuiceRecipe)).setConcealed().setParents(new String[] { "Transmute Honey" }).registerResearchItem();
	}
